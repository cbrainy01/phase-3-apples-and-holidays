...
#  binding.pry
 ...
 while count < seasons.length
  puts "#{seasons[count]}:"
  subcount = 0
  # holidayscount = seasons[count].keys.length

  while subcount < seasons[count].keys.length do
    # prints out the holiday for current season and in that holiday, 
    # iterate through its array and print out its decorations 
    puts"#{seasons[count][subcount]}"
    decoration_count = 0
    while decoration_count < seasons[count][subcount].length do
      puts "#{seasons[count][subcount][decoration_count]}, "
      decoration_count += 1;
    end
